...
function newMessage() {
	...
	if(sessionStorage.getItem("opcaoServico") == "true"){
		if($.trim(message) != '1' && $.trim(message) != '2' && $.trim(message) != '3' && $.trim(message) != '4' || $.trim(message) == ''){
			...
			case "1":	$('<li class="replies"><img src="http://emilcarlsson.se/assets/mikeross.png" alt="" /><p>Ok, para solicitar o guincho, precisamos saber onde te encontrar, compartilhe sua localização conosco</p></li>').appendTo($('.messages ul'));
						$('.message-input input').val(null);
